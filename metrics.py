...
# Returns the intersect over unioun result of binary matrices
def single_pair_weighted_IoU(mask:np.ndarray, prediction:np.ndarray) -> float:
    # Intersections only occurs when has 1 in both matrices, so 2 will be the result in these positions after the sum:
    intersection:np.ndarray = np.where((mask + prediction) == 2, np.ones(mask.shape), np.zeros(mask.shape))
    intersection_count:int = np.count_nonzero(intersection)
    # All places that has 1 or 2 counts as union:
    ...
    union_count:int = np.count_nonzero(union) if np.count_nonzero(union) != 0 else 1
    # If both mask and predictions are blank, then the IoU is maximum:
    IoU:float = 1.0 if (mask == 0).all() and (prediction == 0).all() else intersection_count/union_count
    weight:int = np.count_nonzero(mask)
...
def calculate_metric(metric_name:str, metric_callback:function, class_name:str, predictions_classes:List[np.ndarray], masks_classes:List[np.ndarray], results_file_path:str):
    pairs_total = len(predictions_classes)
    results:List[float] = []
    
    with open(results_file_path, 'a') as file:
        for index in range(pairs_total):
            result = metric_callback(masks_classes[index], predictions_classes[index])
        ...
        file.write('%s of %s segmentation [%.6f]\n' % (metric_name, class_name, sum(results)/pairs_total))
...
def calculate_swIoU(class_name:str, predictions_classes:List[np.ndarray], masks_classes:List[np.ndarray], results_file_path:str):
    pairs_total = len(predictions_classes)
    results:List[float] = []
    ...
    with open(results_file_path, 'a') as file:
        total_weight = 0
        for index in range(pairs_total):
            total_weight += np.count_nonzero(masks_classes[index])
            result = single_pair_weighted_IoU(masks_classes[index], predictions_classes[index])
            results.append(result)
            
            summarizing_result = '> Calculating ' + class_name + ' ' + 'swIoU' + ': pair[%d] - result[%.4f]' % (index, result)
            print(summarizing_result)
            file.write(summarizing_result + '\n')
        file.write('%s of %s segmentation [%.6f]\n' % ('swIoU', class_name, sum(results)/total_weight))